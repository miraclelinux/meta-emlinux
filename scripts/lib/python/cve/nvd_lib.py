...
class NvdCveInfoListCreator:
    ...
    def create_cve_info_list(
        ...
        for src_pkg_name in cve_check_merged_list:
            for cveid in cve_check_merged_list[src_pkg_name]:
                ...
                upstream_version = self.package_info_list.get_upstream_version(
                    src_pkg_name
                )
                ni = self._get_cve_information(
                    cveid, src_pkg_name, binary_package_names, upstream_version, status
                )
    ...
    def _get_cve_information(
        ...
        more_info = f"https://security-tracker.debian.org/tracker/{cveid}"

        if data:
            if data[0] == "Rejected":
                vuln_status = CveInfo.CVE_STATUS_REJECTED
            ...
            more_info = f"https://nvd.nist.gov/vuln/detail/{cveid}"
